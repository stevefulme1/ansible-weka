...
def main():
    ...
    if state == "present":
        existing = None
        if resource_id:
            existing = client.get("cluster_drive", resource_id)
        elif module.params.get("name"):
            candidates = client.list("cluster_drive", {{"name": module.params["name"]}})
            if candidates:
                existing = candidates[0]

        if existing:
            if module.check_mode:
                module.exit_json(changed=False, cluster_drive=existing)
            result = client.update("cluster_drive", resource_id or existing.get("id", ""), module.params)
