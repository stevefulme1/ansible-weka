...
---
module: weka_user
short_description: Manage Weka local users
description:
    - Manage Weka local users.
version_added: "1.0.0"
author:
...
def main():
    module = AnsibleModule(
        ...
        required_if=[("state", "absent", ("user_id",))],
    )
    ...
    if state == "present":
        existing = None
        if resource_id:
            existing = client.get("user", resource_id)
        ...
        if existing:
            ...
            result = client.update("user", resource_id or existing.get("id", ""), module.params)
