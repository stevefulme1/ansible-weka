...
def main():
    ...
    if state == "present":
        existing = None
        if resource_id:
            existing = client.get("tiering_policy", resource_id)
        elif module.params.get("name"):
            candidates = client.list("tiering_policy", {{"name": module.params["name"]}})
            if candidates:
                existing = candidates[0]

        if existing:
            if module.check_mode:
                module.exit_json(changed=False, tiering_policy=existing)
            result = client.update("tiering_policy", resource_id or existing.get("id", ""), module.params)
