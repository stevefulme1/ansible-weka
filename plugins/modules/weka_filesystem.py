...
def main():
    ...
    if state == "present":
        existing = None
        if resource_id:
            existing = client.get("filesystem", resource_id)
        elif module.params.get("name"):
            candidates = client.list("filesystem", {{"name": module.params["name"]}})
            if candidates:
        ...
        if existing:
            ...
            result = client.update("filesystem", resource_id or existing.get("id", ""), module.params)
